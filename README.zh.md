...
- **登录时启动** — 默认开启，可在设置中切换
- **自动 Hook 配置** — 首次启动时通过向导并附带差异预览自动安装 Hook；应用升级后自动检测并提示更新
...
首次启动时，ClaudeNotify 会提示您选择 `settings.json` 文件并自动安装 Hook。应用前会显示差异预览，让您确认具体变更内容。之后也可以随时通过菜单栏安装或卸载 Hook：**设置 > Hook > 安装/卸载 Hook、更改设置文件**。
