...
- **フック自動セットアップ** — 初回起動時に差分プレビュー付きでフックを自動インストール；アプリ更新後は変更を検出してアップデートを案内
...
初回起動時に、ClaudeNotify が `settings.json` ファイルの選択を案内し、フックを自動的にインストールします。適用前に差分プレビューで変更内容を確認できます。後からメニューバーでフックのインストール・アンインストールも可能です：**設定 > フック > フックのインストール/アンインストール、設定ファイルの変更**。
...
- **設定**
  ...
  - フック > フックのインストール/アンインストール、設定ファイルの変更
- **終了**
